...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Examples:"
            echo "  ./scripts/build.sh                 # Build all images"
            echo "  ./scripts/build.sh --api          # Build only API image"
            echo "  ./scripts/build.sh --no-cache     # Build all images without cache"
            exit 0
            ;;
        *)
...
if [ "$BUILD_ALL" = true ] || [ "$BUILD_API" = true ]; then
    ...
    IMAGE_NAME="sec-audit-api"
    if [ -n "$DOCKER_REGISTRY" ]; then
        IMAGE_NAME="$DOCKER_REGISTRY/$IMAGE_NAME"
    fi
    
    docker build $BUILD_ARGS -f Dockerfile.api -t "$IMAGE_NAME:$IMAGE_TAG" .
...
if [ "$BUILD_ALL" = true ] || [ "$BUILD_API" = true ]; then
    if [ -n "$DOCKER_REGISTRY" ]; then
        echo "  - $DOCKER_REGISTRY/sec-audit-api:$IMAGE_TAG"
    else
        echo "  - sec-audit-api:$IMAGE_TAG"
    fi
fi
if [ "$BUILD_ALL" = true ] || [ "$BUILD_WORKER" = true ]; then
    if [ -n "$DOCKER_REGISTRY" ]; then
