...
# or from repo root: PYTHONPATH=backend/src python backend/audit.py path/to/repos.csv ./output
...
Or from `backend/` after `pip install -e .`: `pytest tests/ -v -m "not integration"`.
