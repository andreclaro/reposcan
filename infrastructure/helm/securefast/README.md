...
- Container images built and pushed to a registry:
  - `sec-audit-api:latest` (from `docker/Dockerfile.api`)
...
```bash
# Build API image
docker build -f docker/Dockerfile.api -t your-registry/sec-audit-api:latest .
docker push your-registry/sec-audit-api:latest
...
helm install securefast ./infrastructure/helm/securefast \
  ...
  --set api.image.repository=your-registry/sec-audit-api \
...
api:
  replicaCount: 2
  image:
    repository: my-registry/sec-audit-api
    tag: "v1.0.0"
  autoscaling:
    enabled: true
    minReplicas: 2
