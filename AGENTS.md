...
```
sec-audit-repos/
├── sec_audit/              # Core Python scanning logic
│   ├── cli.py             # CLI entry point
│   ├── scanners.py        # Security scanner wrappers (Semgrep, Trivy, etc.)
│   ├── ecosystem.py       # Language-specific audits (Node, Go, Rust)
│   ├── repos.py           # Git repository operations
│   ├── fs.py              # Filesystem utilities (language detection)
│   ├── utils.py           # Validation and CSV utilities
│   ├── version_manager.py # Runtime version detection/switching
│   └── ai/                # AI analysis modules
│       ├── normalizer.py  # Finding normalization
│       ├── summarizer.py  # AI summary generation
│       ├── storage.py     # Database operations
│       └── storage_backend.py  # S3/local storage
├── api/                    # FastAPI service
│   ├── main.py            # FastAPI app and endpoints
│   └── models.py          # Pydantic request/response models
├── tasks/                  # Celery worker tasks
│   └── scan_worker.py     # Main scan worker implementation
...
- Next.js: http://localhost:3000 (run separately with `pnpm dev` in webapp/)
...
See `webapp/src/db/schema.ts` for complete schema definition.
...
2. Create wrapper function in `sec_audit/scanners.py`
3. Add parser in `sec_audit/ai/parsers/` if normalization needed
4. Integrate into `tasks/scan_worker.py` pipeline
5. Add tests in `tests/test_scanners.py`

### Adding a New Audit Type

1. Add to `ALLOWED_AUDITS` in `sec_audit/utils.py`
2. Add detection logic in `sec_audit/fs.py` or `sec_audit/ecosystem.py`
3. Add execution in `sec_audit/cli.py` and `tasks/scan_worker.py`
4. Update API models in `api/models.py`
5. Update documentation

### Modifying Database Schema

1. Edit `webapp/src/db/schema.ts`
2. Run `pnpm db:generate` to create migration
3. Apply with `pnpm db:migrate`
4. Update Python storage functions in `sec_audit/ai/storage.py`

### Adding AI Analysis Features

1. Modify `sec_audit/ai/summarizer.py` for new analysis types
2. Update database schema if storing new data
3. Update normalizer in `sec_audit/ai/normalizer.py` if needed
...
python sec_audit.py repos.csv ./output --verbose

# Run specific audit only
python sec_audit.py repos.csv ./output --audit sast
```
...
1. **DNS Resolution in Docker**: Check `docker-compose.yml` DNS settings
...
- `design/DESIGN_v0.md`: Full SaaS architecture design
- `design/DESIGN_v0-AI.md`: AI integration design
- `design/DESIGN_SEC_APP_SIMPLE_v0.md`: Simple API backend design
