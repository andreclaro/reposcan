...
The smoke-test file [scripts/repositories_smoke_test.csv](../scripts/repositories_smoke_test.csv) uses the same format as the main repositories list:
...
2. Run sec-audit with the smoke-test CSV and `--audit all`:
   ...
   ```bash
   sec-audit scripts/repositories_smoke_test.csv repos_smoke audit_smoke --audit all
   ```
...
The script [scripts/run_smoke_audit.py](../scripts/run_smoke_audit.py) runs sec-audit with the smoke CSV and then checks that expected output files exist for each repo slug (go, cargo, react, terraform-hello-world).

- **Full run:** `python scripts/run_smoke_audit.py [--csv PATH] [--repos DIR]` — runs sec-audit (repos cloned into `repos_smoke` by default; audit output is `cwd/audit`), then verifies `cwd/audit`.
- **Verify only:** `python scripts/run_smoke_audit.py --verify-only --audit DIR` — only checks that the given audit directory contains the expected files (useful after a manual sec-audit run).
