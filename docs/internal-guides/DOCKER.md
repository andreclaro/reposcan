...
Run the entire Security Audit stack using Docker Compose. Compose and Dockerfiles live in the `docker/` directory.
...
```bash
./infrastructure/deploy/build.sh --api      # Build only API service
./infrastructure/deploy/build.sh --worker   # Build only worker service
./infrastructure/deploy/build.sh --no-cache # Build without cache
```
...
API and worker load variables from `frontend/.env.local` via `env_file`.

**Required:** Create `frontend/.env.local` before starting:
```bash
cp frontend/.env.local.example frontend/.env.local
# Edit with your values
```

Values in `docker/docker-compose.yml` `environment:` block override the file for container networking.
...
- **Worker**: Restart manually: `docker compose -f docker/docker-compose.yml restart worker`
...
# Run Python commands
docker compose -f docker/docker-compose.yml exec api python -c "from api.main import app; print(app)"
```
