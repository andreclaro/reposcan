...
# Allowed audit types (must match sec_audit.utils.ALLOWED_AUDITS minus "all")
...
class ScanRequest(BaseModel):
    ...
    def repo_url_allowed(cls, v: str) -> str:
        from sec_audit.utils import validate_repo_url
    ...
    def branch_valid(cls, v: Optional[str]) -> Optional[str]:
        if v is None or (isinstance(v, str) and not v.strip()):
            return None
        from sec_audit.utils import validate_branch
        try:
